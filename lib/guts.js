...
if (remoteHost === 'auto') {
  ...
  var cmd = 'cd ~/guts && git pull origin master; killall node; killall python; ';
...
function gutUp(done) {
  if (!initted) {
    ...
    exec('gut', ['init'], { cwd: './', pipe: true }, function(err) {
      exec('gut', ['rm', '-r', '--cached', '.'], { cwd: './' }, function(err) {
    ...
    exec('gut', ['remote', 'rm', 'autohost'], { cwd: './' }, function(err) {
      exec('gut', ['remote', 'add', 'autohost', remoteUser + '@' + remoteHost + ':' + remoteFolder], { cwd: './' }, function(err) {
          exec('gut', ['pull', 'autohost', 'master'], { cwd: './', env: { GUT_SSH: path.join(__dirname, 'lib/gut-ssh.sh') } }, function(err) {
          donePulling();
        });
      });
    })
