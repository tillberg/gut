...
if (remoteHost === 'auto') {
  ...
  cmd = cmd + 'killall -q guts -u ' + remoteUser + '; killall -q gutsmon -u ' + remoteUser + '; sleep 2; ';
  cmd = cmd + 'cd ' + remoteFolder + ' && node ~/guts/lib/guts.js ' + localUser + ' auto ' + path.resolve('./');
  console.log(cmd);
  var args = [
     '-L',
     SSH_PORT_LOCAL + ':localhost:' + SSH_PORT_BACKCHANNEL, // expose local 2222 as remote 3295
     '-L',
     HTTP_PORT_LISTEN + ':localhost:' + HTTP_PORT_CONNECT, // expose local 3294 as remote 3296
     '-R',
     HTTP_PORT_LISTEN + ':localhost:' + HTTP_PORT_CONNECT, // expose remote 3294 as local 3296
     '-o',
...
function madeACommit() {
  ...
  exec('curl', ['-s', remoteHost + ':' + HTTP_PORT_CONNECT + '/'], { cwd: './' }, function(err) {});
}
