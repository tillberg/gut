...
process.title = 'guts';
var myname = process.platform === 'darwin' ? 'node' : 'guts';
...
if (remoteHost === 'auto') {
  ...
  var cmd = 'cd ~/guts && git pull origin master; killall -q ' + myname + '; killall -q gutsmon; sleep 1; ';
  ...
  if (!initted) {
    ...
    exec('gut', ['remote', 'rm', 'autohost'], { cwd: './' }, function(err) {
      exec('gut', ['remote', 'add', 'autohost', remoteUser + '@' + remoteHost + ':' + remoteFolder], { cwd: './' }, function(err) {
          exec('gut', ['pull', 'autohost', 'master'], { cwd: './', env: { GUT_SSH: path.join(__dirname, 'gut-ssh.sh') } }, function(err) {
          donePulling();
        });
      });
    })
