...
var SSH_PORT_REMOTE = 22;
var SSH_PORT_LOCAL = 2222;
var SSH_PORT_BACKCHANNEL = 3295;

// problems with ports stuck open?
// sudo fuser -k -n tcp 3296
// sudo fuser -k -n tcp 3295

var remoteUser = process.argv[2];
var remoteHost = process.argv[3];
var remotePort = SSH_PORT_REMOTE;
var remoteFolder = process.argv[4];

var HTTP_PORT_LISTEN = 3294;
var HTTP_PORT_CONNECT = 3296;

if (remoteHost === 'auto') {
  remoteHost = 'localhost';
  remotePort = SSH_PORT_BACKCHANNEL;
  ...
  cmd = cmd + 'killall guts -u ' + remoteUser + '; killall gutsmon -u ' + remoteUser + '; sleep 2; ';
  cmd = cmd + 'cd ' + remoteFolder + ' && node ~/guts/lib/guts.js ' + localUser + ' auto ' + path.resolve('./');
  console.log(cmd);
  var args = [
     '-L',
     HTTP_PORT_CONNECT + ':localhost:' + HTTP_PORT_LISTEN, // expose remote 3294 as local 3296
     '-R',
     HTTP_PORT_CONNECT + ':localhost:' + HTTP_PORT_LISTEN, // expose local 3294 as remote 3296
     '-R',
     SSH_PORT_BACKCHANNEL + ':localhost:' + SSH_PORT_LOCAL, // expose local 2222 as remote 3295
     '-o',
  ...
  info(args.join(' ') + ' ' + cmd);
  var proc = exec('ssh', args, { cwd: './' }, function(err, o, e) {
    console.log('ssh connection terminated. ' + e);
  });
...
function madeACommit() {
  ...
  exec('curl', ['-s', 'localhost:' + HTTP_PORT_CONNECT + '/'], { cwd: './' }, function(err) {});
}
...
function gutUp(done) {
  if (!initted) {
    ...
    exec('gut', ['remote', 'rm', 'origin'], { cwd: './' }, function(err) {
      var url = 'ssh://' + remoteUser + '@' + remoteHost + ':' + remotePort + remoteFolder;
      info('pulling from ' + url);
      exec('gut', ['remote', 'add', 'origin', url], { cwd: './' }, function(err) {
    ...
    removed = [];
    function add() {
      // Add _changed folders, in order of length of the folder path,
      // but don't add any folders where a parent has already been added.
      processed = [];
      function _add(f, cb) {
        if (!_.any(processed, function(p) {
          ...
          info('already added parent of ' + f);
          cb();
        }
      }
      if (_changed.length) {
        var sorted = _.sortBy(_changed, function(f) {
          return f.length;
        });
        async.forEachSeries(sorted, _add, commit);
      } else {
        commit();
      }
    }
